...
export default observer(Vue.component('DatePickerItem', {
  ...
  render(h) {
    ...
    if(type==='date' || type === 'dateTime'){
      ...
      // <!-- 日期 -->
      return (
        <elDatePicker type={type.toLowerCase()} {...commonProps} defaultValue={new Date()} placeholder={placeholder} />
      )
    } else if(type==="dateToDate") {
      // <!-- 日期范围 -->
      const { startPlaceholder, endPlaceholder, time } = itemConfig
      return (
        <elDatePicker type='daterange' {...commonProps} unlinkPanels startPlaceholder={startPlaceholder || "起始时间"} endPlaceholder={endPlaceholder || "截止时间"} pickerOptions={pickerOptions} />
      )
    } else {
      return null
    }
