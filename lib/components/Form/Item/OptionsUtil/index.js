...
function useOptionsStore(itemConfig) {
    // reaction(()=>itemConfig.options, console.log)
    return mobx_react_lite_1.useAsObservableSource(new stores_1.OptionsStore(itemConfig));
}
exports.useOptionsStore = useOptionsStore;
function useOptionsStoreProps(itemConfig, Component) {
    var _a = __read(react_1.default.useState(new stores_1.OptionsStore(itemConfig)), 1), store = _a[0];
    // reaction(()=>itemConfig.options, console.log)
