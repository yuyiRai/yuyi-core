...
var ViewItem = slotInject(function (props) {
    var value = props.value, itemConfig = props.itemConfig;
    var useSlot = itemConfig.useSlot, type = itemConfig.type;
    if (type === 'radioOne') {
    ...
    else if (useSlot !== null && Utils.isFunction(_this.$scopedSlots[useSlot])) {
        // console.log('view item', itemConfig, itemConfig.code,this.$scopedSlots , useSlot)
        return _this.$scopedSlots[useSlot]({ data: value, props: itemConfig.form });
    }
    else {
        return React.createElement("span", null,
            React.createElement("span", null,
