...
const ItemDisplayConfig_1 = require("./ItemDisplayConfig");
const Date_1 = require("./input/Date");
let ItemConfig = ItemConfig_1 = class ItemConfig {
    constructor(initModel, form = {}, componentProps = {}) {
        ...
        this.validateHandler = (value, strict = false) => {
        ...
        this.reaction(() => this.i, (i) => {
            ...
            this.observe(i, (e) => {
                ...
                if (name === '$$core_options' && !Utils_1.Utils.isEqual(oldValue, newValue)) {
                    this.label === '诊断名称' && console.log(`${name}: options[${(oldValue || []).length}] => options[${(newValue || []).length}]`, { config: i, event: e });
                    if (newValue) {
                        this.optionsInited = Utils_1.Utils.isNotEmptyArray(newValue);
                    }
    ...
    observe(value, listener, fireImmediately) {
    ...
    optionsMatcher(r, values, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.allowCreate) {
                const options = yield Reflect.apply(this.getOptionsSafe, this, []);
                for (const value of Utils_1.Utils.isStringFilter(values, '').split(',')) {
