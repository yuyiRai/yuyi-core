...
var OptionsStore = /** @class */ (function () {
    function OptionsStore(itemConfig) {
        ...
        if (this.itemConfig.allowInput) {
            console.log(this);
            this.$on('options-change', function (options) {
                console.log(options, _this.lastOptions, _this.shadowOption);
                _this.lastOptions = lodash_1.cloneDeep(options);
                // if(this.itemConfig.label==='归属车辆')
                //   debugger
                _this.setShadowOption(_this.shadowOption.label, 'options-update');
            });
        ...
        get: function () {
            ...
            while (++index < length) {
                ...
                if (!utils_1.default.isNil(item)) {
                    next[index] = (utils_1.default.isObject(item) ? (item.__key == null ? __assign({}, item, { __key: OptionsStore_1.getOptionsKey(item, index) }) : item) : {
