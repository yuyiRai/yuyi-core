...
const TypeLib_1 = require("src/utils/TypeLib");
 ...
 * @param valueKey
 */
function getCodeListByKey(codeType, valueLabel = false, valueKey = false) {
    const { isStringFilter: isStrF, isArrayFilter } = TypeLib_1.typeFilterUtils;
    ...
    else if (lodash_1.isString(codeType)) {
        return function (keyWord, isOnlySearch) {
            return __awaiter(this, void 0, void 0, function* () {
                const res = yield request_1.default({
                    url: `support/${Utils_1.Utils.isNotEmptyString(keyWord) ? 'findCodeListByName' : 'findListByCodeType'}`,
                    params: {
                        codeType,
                        codeName: (isStrF(keyWord, '') || '').replace(/\[(.*?)\]/, '')
                    }
                });
                return lodash_1.map(isArrayFilter(res, []) || [], ({ codeCode: value, codeName: label, label: nativeLabel }, index) => ({ label: isStrF(label, nativeLabel), value: valueLabel ? label : value, key: valueKey ? value : index }));
            });
        };
    }
    return function () {
...
function getCodeListByTranslate(codeType, valueLabel = false, valueKey = false) {
    const getter = getCodeListByKey(codeType, valueLabel, valueKey);
    return function (codeCode) {
        return __awaiter(this, void 0, void 0, function* () {
            const codeName = yield request_1.default({ url: 'support/codeTranslate', params: { codeType, codeCode } });
            return getter(codeName);
        });
    };
}
exports.getCodeListByTranslate = getCodeListByTranslate;
exports.default = {
    getCodeListByKey,
