...
// respone interceptor
service.interceptors.response.use((_a) => {
    var { data: response } = _a, other = __rest(_a, ["data"]);
    // console.warn({ data: response, ...other })
    if (response && response.success === false) {
        if (response.errorCode === 'unauthorized') {
            element_ui_1.MessageBox.confirm('你已被登出，可以取消继续留在该页面，或者重新登录', '确定登出', {
                confirmButtonText: '重新登录',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                store_1.default.dispatch('FedLogOut').then(() => {
...
const getReq = function (param, resolve, reject) {
    Utils_1.default.simpleTimeBufferInput(param.__res_key, param, function (req) {
        service(Utils_1.default.last(req)).then(function (res) {
            resolve(res);
        }).catch(function (e) {
            reject(e);
        });
...
class HttpBox {
    constructor() {
        ...
        this.getRes = (param) => {
            const { isObjectFilter, createObjectKey } = Utils_1.default;
            ...
            Utils_1.default.createSimpleTimeBufferInput();
        ...
        this.getReq = (param, resolve, reject) => {
            Utils_1.default.simpleTimeBufferInput(param.__res_key, { param, resolve, reject }, this.todo, 20);
        };
        this.setCahce = (__res_key, value) => {
            this._map[__res_key] = value;
        };
        this.getCahce = (__res_key) => {
            return this._map[__res_key];
        };
        this.todo = (list) => {
            const { param: req } = Utils_1.default.last(list);
            service(req).then((res) => {
                ...
                console.log(e, e.message, Utils_1.default.isNotEmptyValueFilter(e.message, e));
                for (const i of list) {
                    i.reject(new Error(Utils_1.default.isNotEmptyValueFilter(e.message, e)));
                }
            });
        };
    }
