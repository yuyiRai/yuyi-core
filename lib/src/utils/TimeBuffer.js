...
function simpleTimeBuffer(time, isDeepDiff, callback, emitter) {
    ...
    if (emitter === void 0) { emitter = new EventEmitter_1.default(); }
    var timeInput = Utils_1.Utils.isNumberFilter(time, 500);
    var $emitter = rxjs_1.from(emitter);
    var $source = rxjs_1.of(null).pipe(operators_1.switchMap(function () { return $emitter; }), isDeepDiff ? timeBufferFactory.deepDiff : timeBufferFactory.diff, operators_1.bufferWhen(function () { return $emitter.pipe(operators_1.switchMap(function () { return rxjs_1.timer(timeInput); })); }), operators_1.first());
    return ([
        emitter,
...
function createSimpleTimeBufferInput(callback, instance, time, isDeepDiff) {
    ...
    return function (value) { return simpleTimeBufferInput(instance, value, callback, time, isDeepDiff); };
}
...
function $message(config, instance, time) {
    simpleTimeBufferInput(instance, config, function (configList) {
        ...
        vue_1.default.prototype.$message(__assign({}, config, { message: lodash_1.join(Array.from(new Set(config.msg)), '<br />') }));
    }, time || 100);
}
exports.$message = $message;
$message.error = function (msg, instance) {
    return $message({ msg: msg, type: 'error' }, instance);
};
function $notify(config, instance, time, h) {
    if (h === void 0) { h = (instance || new vue_1.default({})).$createElement; }
    return simpleTimeBufferInput(instance, config, function (configList) {
        ...
        return Utils_1.Utils.isFunctionFilter(instance && instance.$notify, vue_1.default.prototype.$notify)(__assign({}, config, { message: <span>{lodash_1.reduce(Array.from(new Set(config.msg)), function (list, vdom) { return __spread(list, [list.length > 0 && <br />, Utils_1.Utils.isFunction(vdom) ? vdom() : vdom]); }, [])}</span> }));
    }, time || 100);
}
exports.$notify = $notify;
exports.default = __assign({ $notify: $notify,
    $message: $message,
