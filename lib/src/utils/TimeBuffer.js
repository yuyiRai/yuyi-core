...
export function simpleTimeBuffer(time, isDeepDiff, callback, emitter) {
    ...
    var $source = of(null).pipe(switchMap(function () { return $emitter; }), isDeepDiff ? timeBufferFactory.deepDiff : timeBufferFactory.diff, bufferWhen(function () { return $emitter.pipe(switchMap(function () { return timer(timeInput); })); }), first());
    return ([
        emitter,
...
export function $message(config, instance, time) {
    simpleTimeBufferInput(instance, config, function (configList) {
        ...
        Vue.prototype.$message(__assign({}, config, { message: join(Array.from(new Set(config.msg)), '<br />') }));
    }, time || 100);
}
$message.error = function (msg, instance) {
    return $message({ msg: msg, type: 'error' }, instance);
};
export function $notify(config, instance, time, h) {
    if (h === void 0) { h = (instance || new Vue({})).$createElement; }
    return simpleTimeBufferInput(instance, config, function (configList) {
        ...
        return Utils.isFunctionFilter(instance && instance.$notify, Vue.prototype.$notify)(__assign({}, config, { message: <span>{reduce(Array.from(new Set(config.msg)), function (list, vdom) { return __spread(list, [list.length > 0 && <br />, Utils.isFunction(vdom) ? vdom() : vdom]); }, [])}</span> }));
    }, time || 100);
}
export default __assign({ $notify: $notify,
    $message: $message,
