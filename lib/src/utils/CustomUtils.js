...
export const CustomUtils = {
    ...
    zipEmptyData(object, isRemoveRepeat = true) {
        return isArray(object)
            ? Utils.pipe(filter(object, v => Utils.isNotEmptyValue(v)), (list) => Utils.jsxIf(isRemoveRepeat, Array.from(new Set(list)), list))
            : reduce(filter(keys(object), (k) => Utils.isNotEmptyValue(object[k])), (o, key) => assign(o, { [key]: object[key] }), {});
    },
    ...
    createCommonDto(model) { return new CommonDto(model); },
    /**
    ...
    getExpressByStr(keyStr, defaultValue) {
        ...
        return reduce(paramList, (list, [prototeryName, ...indexList], reduceIndex, reduceList) => {
            return list.concat([
