...
export namespace GulpUtils {
  ...
  export function task<K extends string>(name: K, handler: TaskFunction | string): TaskHandler<K> {
    const func: TaskHandler<K> = Object.assign(handler instanceof Function ? handler : GulpUtils.shellTask(handler), {
      taskName: name
    })
    gulp.task(name, func);
    return func;
  ...
  export function createTemplateStrHandler(haneler: (str: string) => any, paramTo?: FileTemplateRaw<FileParam>) {
    return function templated(msg: FileRawsOptions, ...raws: FileTemplateRaws) {
      return GulpUtils.getThrouth(async function (file, encode, done) {
        const message: string = GulpUtils.convertTemplates(paramTo ? paramTo(file) : file, msg, ...raws);
        // console.log(message)
  ...
  export const shellTask = shellJS.task
  // export function shell(msg: TemplateStringsArray, ...raws: LoggerRaws) {
  //   return GulpUtils.getThrouth((file, encoding, cb) => {
  //     // console.log(file.fileName);
  //     shellJS.task(rawToString([file], msg, ...raws))().then(() => cb());
  //   })
  // }
}
