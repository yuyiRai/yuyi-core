package.path = 'src\\?.lua;src\\TrpgScript\\?.lua;.\\?.lua;lib\\?.lua;.\\?51.dll;utils\\?.lua;lib\\?.lua;src\\utils\\?.lua;'
local testTxt = [[
...
require 'lib.TrpgScript.TextPanel'
...
-- YuyiCore.PrintTable(
--   YuyiCore.splitStr(testTxt, "([??????????????????????????????????????????????????????????????????????]+)", true)
-- )

-- print(YuyiCore.encode('????', 'Shift-jis', 'GB2312'))
