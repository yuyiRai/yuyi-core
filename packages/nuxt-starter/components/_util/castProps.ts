...
export function convertArrayProps<T, Target>(obj: Object | T | T[], obj2Array?: IObject2Array<Target>, deepLimit = 0): Target[] {
  const input = [obj]
  const r = [];
  let nextLevelSize = [];
  let currentIndex = 0;
  ...
  return Object.keys(obj).reduce(
    (arr, key) => {
      if (obj2Array) {
        ...
        var r = obj[key] === true ? obj2Array(key) : (obj[key] && obj2Array(obj[key], key, arr) || false)
        if (r !== false) {
 ...
 * 参照convertArrayProps
 */
export function trueOrArrayProps<T, Input = any>(target: T[] | boolean | Object, defaultValue: any): any {
  return (target === true ? defaultValue : ((target instanceof Object) ? convertArrayProps<T, Input>(target) : []))
}
