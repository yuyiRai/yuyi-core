...
import { filter as lFilter, forEach, reduce, isArray } from '../LodashExtra';
import { convertArr2Map } from '../OptionsUtils';
import { computed, observable } from "./mobx.export";
...
export class EventStore<EventKeys extends string = string> extends EventEmitter<any> implements IEventStore {
  ...
  @computed
  private get _$$eventsNamesMap() {
    var eventNames = this.eventNames
    return eventNames && eventNames.length > 0 ? convertArr2Map(eventNames) : {}
  }
  /**
   * 是否是合法事件名
  ...
  public isValidEventName(eventName: string) {
    return this._$$eventsNamesMap[eventName] || ('*' in this._$$eventsNamesMap)
  }

  constructor(eventNames: string[], public instance: any) {
    super()
   ...
   */
  public $on(eventName: EventKeys, callback: Function, instance: any = this) {
    if (this.isValidEventName(eventName)) {
...
//     return isArray(t.injectedValidEventNames)
//       ? Constant$.OBJ_ASSIGN(r, {
