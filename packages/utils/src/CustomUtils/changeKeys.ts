 ...
 */
export function shiftKeyGroup<T, K extends keyof T, Map extends Record<K, K | string>>(obj: Record<string | number | K, any>, shiftKeysMap: Partial<Map>, shiftMode?: ShiftMode): Record<K, any>;
export function shiftKeyGroup<
  T extends Record<any, any>,
  K extends keyof T,
  Map = Record<string | number | K, string | number | K>
>(obj: any, shiftKeysMap: Map, shifter: (key: string, map: Map) => string | false | void | undefined | null): any;
export function shiftKeyGroup<
...
>(obj: any, shiftKeysMap: Map, replacer: Function | ShiftMode = ShiftMode.Auto) {
  ...
  /** 缓存的替换器返回结果 */
  var replacerResultKey: number | string | null | undefined | false = null
  ...
  Constant$.FOR_EACH(Constant$.OBJ_KEYS(obj), (sourceKey) => {
    ...
    const validTarget = (replacerResultKey === 0 || !!replacerResultKey) && replacerResultKey !== sourceKey
    // console.log(replacerResultKey, sourceKey, validTarget)
    /** 1.需要为合法替换对象 2.对象键是存在于原对象中的 3.没有替换计划 */
    const exchange = replacer === ShiftMode.Auto && validTarget && replaceKeyInSource && !replaceKeyInPlan
    if (validTarget) {
    ...
    if (exchange) {
      ...
      const deleteOrigin = replacerResultKey === false
...
export function shiftKeyTo<
  ...
  SK extends keyof T,
  TK extends keyof T
  >(target: T, sourceKey: SK, targetKey: TK): Exchanged<T, SK, TK>
...
export function shiftKeyTo<
  ...
  SK extends keyof T,
  TK extends keyof T,
  Mode extends ShiftMode.Auto,
  >(target: T, sourceKey: SK, targetKey: TK, mode: Mode): Exchanged<T, SK, TK>
...
shiftKeyTo.cut = function <T, SK extends keyof T, TK extends keyof T>(target: T, sourceKey: SK, targetKey: TK): Shifted<T, SK, TK, ShiftMode.Cut> {
  return shiftKeyTo(target, sourceKey, targetKey, ShiftMode.Cut)
}
shiftKeyTo.copy = function <T, SK extends keyof T, TK extends keyof T>(target: T, sourceKey: SK, targetKey: TK): Shifted<T, SK, TK, ShiftMode.Copy> {
 ...
 */
export type Shifted<
  T, SK extends keyof T,
  TK extends keyof T,
  Mode extends Exclude<ShiftMode, ShiftMode.Auto>,
  TargetKeys extends Mode extends ShiftMode.Cut ? Exclude<keyof T, SK> : keyof T = Mode extends ShiftMode.Cut ? Exclude<keyof T, SK> : keyof T
> = {
