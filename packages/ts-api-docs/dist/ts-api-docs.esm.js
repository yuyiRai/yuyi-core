...
import { src, parallel, dest, series, task as task$1 } from 'gulp';
import { sleep } from '@yuyi919/utils';
import { logger, shell, task, shellTask } from '@yuyi919/gulp-awesome';
import path__default, { join, dirname, relative } from 'path';
import 'glob';
...
import colors__default, { yellow } from 'colors';
import { FileSystem, StringBuilder, InternalError, PackageName, JsonSchema, JsonFile } from '@rushstack/node-core-library';
import { ApiModel, ApiDocumentedItem, ApiItemContainerMixin, ApiParameterListMixin, ApiReleaseTagMixin, ReleaseTag, ApiDeclaredItem, ApiReturnTypeMixin, ApiStaticMixin, ApiPropertyItem, ApiClass, ApiInterface, ApiTypeParameterListMixin } from '@microsoft/api-extractor-model';
import { DocNodeContainer, DocNode, DocSection, DocPlainText, TSDocConfiguration, DocNodeTransforms, DocParagraph, DocFencedCode, StringBuilder as StringBuilder$1, StandardTags, DocCodeSpan, DocLinkTag } from '@microsoft/tsdoc';
...
function createMainApiTask() {
  return function () {
    ...
    return src(filePath).pipe(logger.log('project_name: ' + process.env.DOC_PROJECT_NAME)).pipe(shell("api-extractor run -c " + filePath + " --local"));
  };
}
function createApiTask(folderName) {
  ...
  return function () {
    ...
    return src(filePath).pipe(logger.log('Project Tree Api: ' + folderName)).pipe(shell("api-extractor run -c " + filePath + " --local"));
  };
}
var getEmptyTask = function getEmptyTask(msg) {
  return task('empty', function () {
    try {
      return Promise.resolve(sleep(100)).then(function () {
        return src('.', {
          read: false
        }).pipe(logger.log(msg));
      });
...
(function (ApiExtractorFix) {
  ...
  function taskFactroy(name) {
    var src$1 = resolveTmpDir("./etc/" + (name === projectName ? 'index' : name) + suffix);
    return function () {
      return src(src$1, {
        allowEmpty: true
      }).pipe(replace('@yuyi919/ts-api-docs', name)).pipe(jeditor(function (json) {
        console.error("modify " + src$1 + " name to " + name);
        set(json, 'name', name);
        return json;
      })).pipe(dest(paths.tmpEtcDir));
    };
  }
  ...
  function editYml(source, target, dothing) {
    return src(source).pipe(convert({
      // @ts-ignore
      from: 'yml',
      to: 'json'
  ...
  function createApiFixedTask(jsonTmp) {
    ...
    var markdownList = [];
    var indexFile = "";
    return parallel(series(function templateInit() {
      ...
      return src(resolve('./config/template/apiDoc/**/*')).pipe(dest(resolveTmpDir('./document')));
...
function () {
  ...
  _proto.buildApiModel = function buildApiModel() {
    ...
    for (var _iterator = FileSystem.readFolder(this.inputFolder), _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
      var _ref;
      ...
      if (filename.match(/\.api\.json$/i)) {
        ...
        var filenamePath = join(this.inputFolder, filename);
      ...
      if (apiItem.tsdocComment) {
        ...
        if (inheritDocTag && inheritDocTag.declarationReference) {
          ...
          if (result.errorMessage) {
            console.log(yellow("Warning: Unresolved @inheritDoc tag for " + apiItem.displayName + ": " + result.errorMessage));
          } else {
            if (result.resolvedApiItem instanceof ApiDocumentedItem && result.resolvedApiItem.tsdocComment && result.resolvedApiItem !== apiItem) {
  ...
  _createClass(CustomDocNodes, null, [{
    ...
    get: function get() {
      if (CustomDocNodes._configuration === undefined) {
        var configuration = new TSDocConfiguration();
...
function () {
  function IndentedWriter(builder) {
    ...
    this._$$_builder = builder === undefined ? new StringBuilder() : builder;
      ...
      case DocNodeKind$1.Paragraph:
        {
          ...
          var trimmedParagraph = DocNodeTransforms.trimSpacesInParagraph(docParagraph);
...
function () {
  ...
  _proto.load = function load(documenterConfig, createContext) {
    var configFileFolder = dirname(documenterConfig.configFilePath);
    ...
    for (var _iterator = documenterConfig.configFile.plugins || [], _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
      ...
      try {
        ...
        var resolvedEntryPointPath = sync(configPlugin.packageName, {
...
function () {
  ...
  _proto._$$_writeApiItemPage = function _$$_writeApiItemPage(apiItem) {
    ...
    switch (apiItem.kind) {
      ...
      :
        ...
        var unscopedPackageName = PackageName.getUnscopedName(apiItem.displayName);
        output.appendNode(new DocHeading({
          configuration: configuration,
    ...
    if (apiItem instanceof ApiDeclaredItem) {
      if (apiItem.excerpt.text.length > 0) {
        output.appendNode(new DocParagraph({
          configuration: configuration
        }, [new DocEmphasisSpan({
          configuration: configuration,
          bold: true
    ...
    switch (apiItem.kind) {
      ...
      :
        ...
        if (_tsdocComment) {
          ...
          if (codeSpinnet) {
            this._$$_appendSection(output, new DocSection({
              configuration: this._$$_tsdocConfiguration
            }, [codeSpinnet]));
          }
        }
    ...
    var filename = join(this._$$_outputFolder, this._$$_getFilenameForApiItem(apiItem));
  ...
  _proto._$$getSummaryConnect = function _$$getSummaryConnect(output, section, parent) {
    ...
    if (section.kind === DocNodeKind$1.BlockTag && section._tagName === '@includeSnippet') {
      ...
      if (next.kind === DocNodeKind$1.PlainText) {
        ...
        if (codePath) {
          var filePath = relative(process.cwd(), codePath.replace(/^( )+/, '').replace('~', this.projectFolder));
          console.log( // includeSpinning.configuration.tagDefinitions,
          includeSpinning.kind, text, filePath);
          var readFile = FileSystem.readFile(filePath); // const code = /test\(['"](.*?)['"], (.[\s\S]*)\)/.exec(readFile);
          // console.log(code);
  ...
  _proto._$$_writeParameterTables = function _$$_writeParameterTables(output, apiParameterListMixin) {
    ...
    for (var _iterator9 = apiParameterListMixin.parameters, _isArray9 = Array.isArray(_iterator9), _i9 = 0, _iterator9 = _isArray9 ? _iterator9 : _iterator9[Symbol.iterator]();;) {
      ...
      var parameterDescription = new DocSection({
        configuration: configuration
      });
  ...
  _proto._$$_createModifiersCell = function _$$_createModifiersCell(apiItem) {
    ...
    if (ApiStaticMixin.isBaseClassOf(apiItem)) {
      if (apiItem.isStatic) {
        section.appendNodeInParagraph(new DocCodeSpan({
          configuration: configuration,
          code: 'static'
        }));
      }
  ...
  _proto._$$_writeBreadcrumb = function _$$_writeBreadcrumb(output, apiItem) {
    ...
    for (var _iterator10 = apiItem.getHierarchy(), _isArray10 = Array.isArray(_iterator10), _i10 = 0, _iterator10 = _isArray10 ? _iterator10 : _iterator10[Symbol.iterator]();;) {
      ...
      switch (hierarchyItem.kind) {
        ...
        default:
          output.appendNodesInParagraph([new DocPlainText({
            configuration: this._$$_tsdocConfiguration,
            text: ' > '
          }), new DocLinkTag({
            configuration: this._$$_tsdocConfiguration,
  ...
  _proto._$$_deleteOldOutputFiles = function _$$_deleteOldOutputFiles() {
    ...
    FileSystem.ensureEmptyFolder(this._$$_outputFolder);
  };
...
JsonSchema.fromFile(
/*#__PURE__*/
join(__dirname, '..', 'yaml', 'typescript.schema.json'));
var Navigation = {
  Exports: ".",
  Members: "#",
  Locals: "~"
...
function () {
  ...
  _proto._$$_writeTocFile = function _$$_writeTocFile(apiItems) {
    ...
    var tocFilePath = join(this._$$_outputFolder, 'toc.yml');
  ...
  _proto._$$_getYamlFilePath = function _$$_getYamlFilePath(apiItem) {
    ...
    return join(this._$$_outputFolder, result + disambiguator + '.yml');
  };
...
function init() {
  ...
  task('api:fix', apiFix);
  task('doc:fix', docFix);
  var build = task('build', function () {
    src('.', {
      read: false
    }).pipe(logger('Exec')("api-documenter markdown -i " + resolveTmpDir("./etc") + " -o " + resolveTmpDir("./document/articles"))).pipe(logger('Exec')("api-documenter yaml -i " + resolveTmpDir("./etc") + " -o " + resolveTmpDir("./document/src"))).pipe(logger('Exec')("docfx build " + resolveTmpDir('./document/docfx.json'))).pipe(shell("docfx build " + resolveTmpDir('./document/docfx.json'))).pipe(logger('Run server with:')(colors__default.cyan(colors__default.underline("docfx serve ./docs"))));
  });
  var tmpTemplate = task('doc:templateTmp', function () {
    return src(resolve('./config/document/**')).pipe(replace('"../docs"', "\"" + resolve('./docs', true) + "\"")).pipe(dest(resolveTmpDir('./document')));
  });
  var cleanDocTmp = task('clean:doctmp', shellTask("rimraf " + resolveTmpDir('./document/**')));
  var outputTemplate = task('doc:template', function () {
    return src(resolve('./config/document/**')).pipe(dest('./document'));
  });
  task$1('default', series( // apiMain,
  apiTree, // (async (done) => {
  //   const tasks = [];
  ...
  // }),
  apiFix, cleanDocTmp, tmpTemplate, parallel(task('markdown:articles', function () {
    try {
...
export { MarkdownAction, MarkdownDocumenterAccessor, MarkdownDocumenterFeature, MarkdownDocumenterFeatureContext, PluginFeature, PluginFeatureContext, PluginFeatureInitialization, YamlAction, init };
//# sourceMappingURL=ts-api-docs.esm.js.map
