...
import { ApiItemKind, DocNodeKind, NewlineKind } from './type';
...
export class MarkdownDocumenter {
  ...
  public constructor(apiModel: ApiModel, public projectFolder?: string, documenterConfig?: DocumenterConfig) {
  ...
  private _writeApiItemPage(apiItem: ApiItem): void {
    ...
    switch (apiItem.kind) {
      case ApiItemKind.Class:
        const tsdocComment: DocComment | undefined = (apiItem as ApiClass).tsdocComment;
        if (tsdocComment) {
          const codeSpinnet = this.getSummaryConnect(output, tsdocComment.summarySection)
          if (codeSpinnet) {
            this._appendSection(output, new DocSection({
              configuration: this._tsdocConfiguration
            }, [codeSpinnet]))
          }
        }
    ...
    FileSystem.writeFile(filename, pageContent, {
      // @ts-ignore
      vertLineEndings: this._documenterConfig ? this._documenterConfig.newlineKind : NewlineKind.CrLf
    });
  }
   ...
   * @param parent 
   */
  private getSummaryConnect(output: DocSection, section: DocSection, parent: DocSection[] = []) {
    if (!section) return null
    if (section.kind === DocNodeKind.BlockTag && (section as any)._tagName === '@includeSnippet' as any) {
      // if (section) {
      const includeSpinning: DocBlock = section as any
      const parentNode = parent[parent.length - 1]
      const next = parentNode.getChildNodes()[parentNode.getChildNodes().findIndex(i => i === section) + 1]
      if (next.kind === DocNodeKind.PlainText) {
        const { text } = next as DocPlainText
        const [codePath, name] = text.split('#')
        if (codePath) {
          const filePath = path.relative(process.cwd(), codePath.replace(/^( )+/, '').replace('~', this.projectFolder));
          console.log(
            // includeSpinning.configuration.tagDefinitions,
            includeSpinning.kind,
            text, filePath
          );
      ...
      // (next as DocPlainText)
      // parentNode.clearNodes()
      // this.getSummaryConnect(output, includeSpinning.content, [...parent, section])
      // this._appendSection(output, parent[parent.length - 1])
      // output.appendNode(new DocHeading({ configuration: section.configuration, title: `@includeSnippet enum` }));
    } else if (section.kind === DocNodeKind.Section || section.kind === 'Paragraph' || section.kind === 'SoftBreak') {
      for (const item of section.getChildNodes()) {
        const r = this.getSummaryConnect(output, item as DocSection, [...parent, section])
        if (r) return r
      }
    }
    return null
