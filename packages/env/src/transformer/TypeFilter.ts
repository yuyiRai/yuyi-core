...
export function mapChildren(children: ts.NodeArray<any> | ts.Node[], times = 1) {
  if (times !== 0) {
    return (children as any).map(c => mapChildren(c.getChildren(), times - 1));
  } else {
    return (children as any).map(c => c.getText())
  }
}

export function visitImportNode(node: ts.Node, program: ts.Program, context: ts.TransformationContext) {
