...
import { action, autorun, computed, intercept, isObservableProp, observable, observe, reaction, toJS } from "mobx";
import { createTransformer } from 'mobx-utils';
export var ExportUtils = {
    export: createTransformer(function (instance) {
        var model = {};
...
var CommonStore = /** @class */ (function (_super) {
    ...
    CommonStore.prototype.onceReaction = function (source, callback, options) {
        var a = this.reaction(source, function (arg, r) {
            callback(arg, r);
            a();
        }, options);
    };
    ...
    CommonStore.prototype.registerDisposer = function (r) {
        var _this = this;
        this.destorySet.add(r);
        return (function () {
            _this.destorySet.delete(r);
            r();
        });
    };
    ...
    CommonStore.prototype.mapToDiff = function (map, source, cahce, deepClone) {
        // console.log('map', source)
        var useCahce = Utils.isObject(cahce);
        var push = difference(keys(source), Array.from(map.keys()));
        forEach(map.toJSON(), function (value, key) {
            if (isNil(source[key])) {
                map.delete(key);
                if (useCahce) {
                    unset(cahce, key);
    ...
    CommonStore.prototype.export = function () {
        return ExportUtils.export(this);
    };
    __decorate([
        observable,
    ...
    ], CommonStore.prototype, "export", null);
    return CommonStore;
}(EventStoreProvider));
export { CommonStore };
